...
type LaptopStore interface{
	// Save stores the laptop in the store
	Save(laptop *pb.Laptop) error
	// Find locates a laptop by ID
...
func isQualified(filter *pb.Filter, laptop *pb.Laptop) bool {
	...
	if laptop.GetCpu().GetCoreCount() < filter.GetMinCpuCores() {
		return false
	}

	if laptop.GetCpu().GetCoreFrequencyGhz() < filter.GetMinCpuGhz() {
		return false
	}
